  ...
  return (
    <div className="border-solid border-2 rounded-md my-4 mx-4 px-2 py-2">
      <div className="flex flex-wrap">
        <div>
          <div className="text-xl text-sky-700 font-semibold mr-4">{repo.name}</div>
        </div>
        <div className="text-xs border border-solid border-gray-400 rounded-full bg-white text-center px-2 py-1">
          {repo.visibility}
        </div>
      </div>
      <div className="text-sm text-gray-700">{repo.description}</div>
      <div className="flex flex-wrap space-x-4 text-sm text-gray-700">
        <div className="font-semibold text-gray-800">{repo.language}</div>
        <div className="flex">
  ...
  return (
    <div className="grid grid-cols-3 my-4">
      ...
      <div className="col-start-1 mt-4 mx-4">
        <div className="mx-2 my-4">{users.bio}</div>
        <div className="flex flex-wrap">
      ...
      <div className="col-start-2 col-span-2 mt-4">
        <div className="ml-4">
          ...
          <div className="flex">
            Public repos:
            <dir className="font-semibold px-1 my-0">{users.public_repos}</dir>
          </div>
          <div className="flex">
            Public gists:
            <dir className="font-semibold px-1 my-0">{users.public_gists}</dir>
          </div>
        </div>
...
function UserPage() {
  ...
  return (
    <div className="bg-violet-300">
      <Navbar />
      {users !== null && <UserDetails users={users} />}
    </div>
  );
}
